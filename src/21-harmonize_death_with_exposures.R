...
paths$input <- list(
  ...
  stmf = './dat/stmf/12-stmf.rds',
  # path to raw ons death data
  ons = './dat/ons/12-ons_annual_deaths.rds',
  # path to raw cdc death data
  cdc = './dat/cdc/Underlying Cause of Death, 1999-2020.txt',
...
# cdc death counts
dat$cdc <- read_tsv(paths$input$cdc)
...
dat$ungrouped_by_unique_age_pattern <-
  ...
  group_modify(~{
    ...
    ungrouped_deaths <- tryCatch({
      ...
      ungrouped[101] <- sum(dx_100p*population_py[101])
